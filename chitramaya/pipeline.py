...
class Pipeline:
    ...
    def __post_init__(self) -> None:
        ...
        self.async_encoder_queue: int = int(self.cfg.get("encoder", "async_encoder_queue", default=32))

        # Async restorer thread (Step 2 threading): overlap BasicVSR++
        # restoration + composite with main thread's decode/detect/track.
        self.async_restorer: bool = bool(self.cfg.get("restoration", "async_restorer", default=True))
        self.async_restorer_queue: int = int(self.cfg.get("restoration", "async_restorer_queue", default=4))
    ...
    def run(self) -> None:
        ...
        if self.async_encoder:
            ...
            encoder = AsyncEncoder(encoder, queue_size=self.async_encoder_queue)
        ...
        # Wrap restorer in AsyncRestorer if enabled. The async wrapper runs
        # restore_clip + composite on a worker thread, overlapping with
        # main thread's decode/detect/track. main thread tracks
        # async_restorer.min_pending_frame() to know when frames are safe
        # to drain to the encoder.
        if self.async_restorer and restorer is not None:
            print(
                f"[Restorer] Async restorer thread enabled "
                f"(queue_size={self.async_restorer_queue})"
            )
            restorer = AsyncRestorer(
                restorer=restorer,
                store=store,
                composite_fn=composite_clip_into_store,
                device=self.device,
                queue_size=self.async_restorer_queue,
            )
            ...
            for i, bgr_u8 in enumerate(batch_bgr_u8):
                ...
                if step.new_clips and restorer is not None:
                    for clip in step.new_clips:
                        t0 = time.perf_counter()
                        if isinstance(restorer, AsyncRestorer):
                            # Async: hand off to worker thread. The submit
                            # may block if the clip queue is full
                            # (backpressure). Frames will be marked
                            # "restored" as the worker completes them — we
                            # update the metric set HERE because we know
                            # the clip will be processed eventually, and
                            # this matches the semantics of the sync path.
                            restorer.submit(clip)
                        else:
                ...
                # safe_before = min(tracker watermark, async restorer watermark, frame_num+1)
                min_start = tracker.min_active_start()
                tracker_safe = int(min_start) if min_start is not None else int(frame_num + 1)
                if isinstance(restorer, AsyncRestorer):
                    restorer_safe = restorer.min_pending_frame()
                else:
                    restorer_safe = (1 << 62)
                safe_before = min(tracker_safe, restorer_safe, int(frame_num + 1))
        ...
        try:
            if use_thread_prefetch:
                ...
                while True:
                    ...
                    while store.is_full():
                        ...
                        # CRITICAL: include async restorer's pending watermark — clips already submitted
                        # to the restorer have been removed from tracker._scenes, so tracker.min_active_start()
                        # alone would let us drain frames whose composite hasn't run yet (race → unrestored output).
                        ...
                        if isinstance(restorer, AsyncRestorer):
                            restorer_sb = restorer.min_pending_frame()
                        else:
                            restorer_sb = (1 << 62)
                        sb = min(tracker_sb, restorer_sb)
            ...
            # If async restorer is in use, drain its queue so all clips
            # finish compositing before we do the final drain to encoder.
            if isinstance(restorer, AsyncRestorer):
                restorer.flush_and_join()
                print(
                    f"[Restorer] Async worker: restored={restorer.clips_processed} clips "
                    f"in {restorer.worker_wall:.2f}s wall (overlapping with main thread)"
                )
