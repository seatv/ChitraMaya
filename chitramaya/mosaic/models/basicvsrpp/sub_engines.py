# ChitraMaya/mosaic/models/basicvsrpp/sub_engines.py
"""BasicVSR++ TensorRT sub-engines: compile + runtime orchestration.

Splits BasicVSR++ into 6 separately-compiled TensorRT engines:
  * 4 loop-body engines (one per propagation direction; static batch=1)
  * 1 preprocess engine (feat_extract + downsample + bidirectional SPyNet)
  * 1 upsample engine (reconstruction + 2x upsample + conv_hr + conv_last)

The split exists because torch_tensorrt's dynamo backend can't compile
BasicVSR++'s recurrent propagation loop as a single graph — Python control
flow and stateful iteration break the export. We compile the purely
feedforward inner step (loop body) and orchestrate the recurrence in
...
def compile_basicvsrpp_sub_engines(
    ...
    # TRT build workspace. The builder uses this as a scratch budget for tactic
    # selection; a bigger budget lets it pick faster, more memory-hungry tactics
    # whose runtime execution context then reserves that memory *resident* for
    # the engine's whole life. The unbounded default (get_workspace_size_bytes:
    # 95% of free) therefore made the sub-engines reserve multiple GB resident,
    # which on an 8 GB card left no room for the frame store at 4K/8K.
    #
    # ChitraMaya's design constraint is VRAM-only: everything stays resident on the
    # GPU, no host-RAM offload (the eGPU runs over a Gen1 x4 link where offload
    # would bottleneck). So we cannot relieve VRAM pressure at runtime the way
    # lada/Jasna do — we must PREVENT it at compile time by bounding the
    # workspace here. Do NOT reintroduce a runtime offloader as a workaround.
    #
    # `workspace_gb` is threaded from the compile CLI (--workspace); when None we
    # fall back to the historical 95%-of-free for backward compatibility.
    if workspace_gb is not None and int(workspace_gb) > 0:
        workspace_size = int(workspace_gb) * 1024 * 1024 * 1024
    else:
