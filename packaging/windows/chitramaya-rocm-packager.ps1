# packaging/windows/chitramaya-rocm-packager.ps1
#
# ROCm (AMD Radeon) edition of chitramaya-packager.ps1. Same ritual as the
# XPU packager, same installer output shape -- differences:
#   * builds packaging\windows\chitramaya-rocm.spec (torch +rocm venv
#     asserted by the spec itself; ffmpeg REQUIRED and AMF-probed;
#     HIP runtime DLL present-or-fatal)
#   * no TensorRT engine steps: engines do not exist on this edition
#   * installer artifacts are named ChitraMaya-rocm-install.* so the
#     three editions' release assets can never be confused in an upload
#
# Run from the repo root, inside the ROCm venv (requirements-rocm.txt,
# python 3.12). The build machine does NOT need an AMD GPU (blind-build
# workflow, validated on the Arc edition) -- but the TARGET machine needs
# the Adrenalin driver 26.2.2+.
#
# After building, sanity-check the dist BEFORE packaging by running:
#   .\dist\ChitraMaya\ChitraMaya-cli.exe -self-check
# On a build machine with no AMD card, expect PASS with a "no rocm
# device visible" WARNING -- any FAIL means the bundle is broken.

param(
  [string]$Name = "ChitraMaya",
  [switch]$SkipFfmpeg = $false,
  [switch]$SwapPolarsLtsCpu = $true,
  [string]$FfmpegDir = "",   # r4: folder containing the ffmpeg.exe/ffprobe.exe
                             # to bundle. Prepended to PATH for this run so the
                             # spec bundles EXACTLY this build instead of
                             # whatever another tool put first on PATH (field
                             # event: a stray C:\MyPrograms\<other-tool>\
                             # ffmpeg.exe was winning the PATH race).
...
# r3: hide HIP devices for the WHOLE packaging run (PyInstaller children,
# hooks, and the post-build self-check all inherit this). On a driverless
# build machine the ROCm runtime natively aborts on any GPU touch; with
# devices hidden, stacks that honor the variable short-circuit cleanly.
# Build-time only -- the shipped app never sets this.
$env:HIP_VISIBLE_DEVICES = ""
$env:ROCR_VISIBLE_DEVICES = ""

Write-Host "== ChitraMaya packaging (ROCm / AMD Radeon) ==" -ForegroundColor Cyan
Write-Host "Name: $Name" -ForegroundColor Cyan
Write-Host "Repo: $(Get-Location)" -ForegroundColor Cyan

# ── Sanity: run from repo root, in the ROCm release venv ─────────────────
...
if (-not (Test-Path ".\packaging\windows\chitramaya-rocm.spec")) { throw "Missing .\packaging\windows\chitramaya-rocm.spec" }
if (-not (Test-Path ".\packaging\windows\chitramaya_entrypoint.py")) { throw "Missing packaging entrypoint." }

if (-not $env:VIRTUAL_ENV) {
  Write-Warning "No active virtualenv detected. Build from the ROCm venv (torch +rocm, requirements-rocm.txt). The spec will refuse a non-rocm torch."
}
...
if (-not $SkipFfmpeg) {
  ...
  if (-not ($ff -and $fp)) {
    throw "ffmpeg.exe/ffprobe.exe not on PATH. On the ROCm edition ffmpeg IS the decoder and encoder. Pass -FfmpegDir <folder with the gyan.dev 'full' build>, or put it first on PATH (the spec bundles exactly what it finds and requires the AMF encoders)."
  }
...
# ── polars AVX guard (same portability concern as the other builds) ──────
if ($SwapPolarsLtsCpu) {
...
Write-Host "Running PyInstaller (torch+rocm stack; expect several GB)..." -ForegroundColor Cyan
python -m PyInstaller --noconfirm --clean .\packaging\windows\chitramaya-rocm.spec -- $specArgs
...
("Place source model files here:" + [Environment]::NewLine +
 ...
 "The AMD (ROCm) edition runs models directly in PyTorch;" + [Environment]::NewLine +
...
# ── Post-build self-check (Batch 34): catch a broken bundle HERE ─────────
# Runs the CLI exe's -self-check. On a build box without an AMD card a
# "no device" WARNING is expected and fine; a FAIL exit code stops the
# packager before a broken build gets archived and shipped to a tester.
$cliExe = Join-Path $distDir "$Name-cli.exe"
if (Test-Path $cliExe) {
  Write-Host "Running post-build self-check..." -ForegroundColor Cyan
  & $cliExe -self-check
  if ($LASTEXITCODE -ne 0) {
    throw "Post-build self-check FAILED (exit $LASTEXITCODE). The dist is broken -- do not package it. Scroll up for the FAIL lines."
  }
  Write-Host "Self-check passed." -ForegroundColor Green
} else {
  Write-Warning "CLI exe not found at $cliExe -- skipping post-build self-check."
}

# ── Release artifact ─────────────────────────────────────────────────────
# Two shapes, chosen by what actually fits -- both are ONE double-click
# installer exe (r6: consistent install experience across editions):
#   FITS under GitHub's 2GB asset limit  -> ONE file:
#       ChitraMaya-rocm-install.exe = 7z.sfx extract-dialog stub + the
#       archive. Double-click, pick a folder, extracts. (Falls back to a
#       plain .7z only if 7z.sfx is not installed.)
#   DOES NOT FIT -> split volumes + ChitraMaya-rocm-install.exe shepherd
#       SFX that verifies the volumes before extracting
#       (vendor setup: packaging\windows\vendor\7zSD.sfx + 7zr.exe).
Write-Host "Creating release artifact..." -ForegroundColor Yellow
if (Get-Command 7z -ErrorAction SilentlyContinue) {
  $installBase = "ChitraMaya-rocm-install"
    ...
    if ($SplitMB -gt 0) {
      ...
      if ($LASTEXITCODE -ne 0) { Write-Warning "Archive creation failed."; $needSplit = $null }
        ...
        if ($mb -le $limitMB) {
          ...
          # release asset (the one-file directive from the first Arc
          # installer round), but double-clickable. 7z.sfx is 7-Zip's
          ...
          if ($sfxGui) {
            ...
            try {
              ...
              Write-Host "and it extracts there -- same experience as the NVIDIA installer." -ForegroundColor Cyan
            } catch {
    ...
    if ($needSplit -eq $true) {
      if (-not (Test-Path $sfxModule) -or -not (Test-Path $sevenZr)) {
        ...
        if ($LASTEXITCODE -ne 0) { Write-Warning "Archive creation failed."; }
          ...
          # >6000 MB dist could not compress under the 2GB asset limit; the
          # ROCm stack then landed at 1801 MB in ONE volume, shipping a
          # shepherd exe + one part where a single-file installer would do.
          # A single volume IS the complete archive, so fold it back into
          # the one-file installer instead of guessing better.
          $foldedSingle = $false
          ...
          # Stamp BOTH the part count AND the base name (the first-xpu-
          # installer field bug: unstamped, install.ps1 hunts for the
          # NVIDIA names forever). Regexes match any prior stamp.
...
Write-Host "Tester: have them run  $Name-cli.exe -self-check  and send you the output." -ForegroundColor Cyan
